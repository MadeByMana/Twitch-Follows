...
const handler: Handler = async (event) => {
  ...
  try {
    ...
    if (token) {
      ...
      console.log('[Netlify Proxy] Using Authenticated Mobile Client ID (No Integrity Needed)');
    } else {
    ...
    try {
      ...
      if (hasAuthError && requestHeaders['Authorization']) {
        ...
        try {
          console.log('[Netlify Proxy] Falling back to unauthenticated Mobile Client ID due to auth error...');
